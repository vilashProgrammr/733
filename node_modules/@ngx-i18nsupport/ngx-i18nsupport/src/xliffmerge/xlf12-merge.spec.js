...
describe('XliffMerge XLIFF 1.2 format tests', () => {
    ...
    describe('Merge process checks for format xlf', () => {
        ...
        it('should fix source language, if the masters lang is not the default', (done) => {
            ...
            const newmaster = readXliff(MASTER);
        ...
        it('should generate translated file for default language de from master', (done) => {
            ...
            xliffMergeCmd.run();
            expect(ws.writtenData()).not.toContain('ERROR');
            const langFile = readXliff(xliffMergeCmd.generatedI18nFile('de'));
        ...
        it('should generate translated file for all languages', (done) => {
            ...
            const langFileEnglish = readXliff(xliffMergeCmd.generatedI18nFile('en'));
        ...
        it('should merge translated file for all languages', (done) => {
            ...
            const tu = langFileEnglish.transUnitWithId(ID_TRANSLATED_SCHLIESSEN);
        ...
        it('should merge missing source refs to already translated files', (done) => {
            ...
            const langFileEnglish = readXliff(xliffMergeCmd.generatedI18nFile('en'));
            const tu = langFileEnglish.transUnitWithId(ID_MISSING_SOURCEREF);
            expect(tu).toBeTruthy();
        ...
        it('should preserve order when merging new units (#96)', (done) => {
            file_util_1.FileUtil.copy(SRCDIR + 'preserveOrderMaster1.xlf', MASTER);
            let ws = new writer_to_string_1.WriterToString();
            let commandOut = new command_output_1.CommandOutput(ws);
            const profileContent = {
                xliffmergeOptions: {
            ...
            file_util_1.FileUtil.copy(SRCDIR + 'preserveOrderMaster2.xlf', MASTER);
            ws = new writer_to_string_1.WriterToString();
            commandOut = new command_output_1.CommandOutput(ws);
        ...
        it('should translate messages with 2 custom tags with different ids (#84)', (done) => {
            ...
            // now translate some texts in the English version
            let langFileEnglish = readXliff(xliffMergeCmd.generatedI18nFile('en'));
            const tu = langFileEnglish.transUnitWithId(ID_2_CUSTOM_TAGS);
            expect(tu).toBeTruthy();
        ...
        it('should translate ICU message with placeholder (#83)', (done) => {
            ...
            // now translate some texts in the English version
            let langFileEnglish = readXliff(xliffMergeCmd.generatedI18nFile('en'));
            const tu = langFileEnglish.transUnitWithId(ID_ICU_WITH_PLACEHOLDER);
            expect(tu).toBeTruthy();
        ...
        it('allowIdChange feature should merge only white space changed content with changed ID' +
            ...
            file_util_1.FileUtil.copy(SRCDIR + 'ngExtractedMasterWithIdChange.xlf', MASTER);
            file_util_1.FileUtil.copy(SRCDIR + TRANSLATED_FILE, WORKDIR + 'messages.en.xlf');
            const ws = new writer_to_string_1.WriterToString();
            const commandOut = new command_output_1.CommandOutput(ws);
            const profileContent = {
        ...
        it('allowIdChange feature with added ICU message (#138)', (done) => {
            ...
            file_util_1.FileUtil.copy(SRCDIR + 'issue138IdChangeICUMaster.xlf', MASTER);
            file_util_1.FileUtil.copy(SRCDIR + TRANSLATED_FILE, WORKDIR + 'messages.de.xlf');
            const ws = new writer_to_string_1.WriterToString();
            const commandOut = new command_output_1.CommandOutput(ws);
            const profileContent = {
            ...
            const xliffMergeCmd = xliff_merge_1.XliffMerge.createFromOptions(commandOut, { languages: ['en', 'de'] }, profileContent);
            xliffMergeCmd.run();
            expect(ws.writtenData()).not.toContain('ERROR');
            ...
            const langFileGerman = readXliff(xliffMergeCmd.generatedI18nFile('de'));
            file_util_1.FileUtil.copy(xliffMergeCmd.generatedI18nFile('de'), 'test/' + 'issue138.de.xlf'); // TODO delete this line
            expect(langFileGerman.numberOfTransUnits()).toBe(6);
            const tuOldIcuRef = langFileGerman.transUnitWithId(ID_OLD_ICUREF);
            expect(tuOldIcuRef).toBeTruthy();
            const tuOldIcu = langFileGerman.transUnitWithId(ID_OLD_ICU);
            expect(tuOldIcu).toBeTruthy();
            expect(tuOldIcuRef.targetContent()).toContain('<x id="ICU" equiv-text="{ fruit, select, Windstorm {...}');
            expect(tuOldIcuRef.targetState()).toBe(ngx_i18nsupport_lib_1.STATE_TRANSLATED);
            expect(tuOldIcu.sourceContent()).toContain('Windstorm {Windstorm}');
            expect(tuOldIcu.targetContent()).toContain('Windstorm {Stürmer}');
            expect(tuOldIcu.targetState()).toBe(ngx_i18nsupport_lib_1.STATE_TRANSLATED);
            const tuChangedIcuRef = langFileGerman.transUnitWithId(ID_CHANGED_ICUREF);
            expect(tuChangedIcuRef).toBeTruthy();
            const tuChangedIcu = langFileGerman.transUnitWithId(ID_CHANGED_ICU);
            expect(tuChangedIcu).toBeTruthy();
            expect(tuChangedIcuRef.targetContent()).toContain('<x id="ICU" equiv-text="{ choice, select, A {...}');
            expect(tuChangedIcuRef.targetState()).toBe(ngx_i18nsupport_lib_1.STATE_TRANSLATED);
            expect(tuChangedIcu.sourceContent()).toContain('A {A}');
            expect(tuChangedIcu.targetContent()).toContain('A {A1}');
            expect(tuChangedIcu.targetState()).toBe(ngx_i18nsupport_lib_1.STATE_TRANSLATED);
            const tuAddedIcuRef = langFileGerman.transUnitWithId(ID_ADDDED_ICUREF);
            expect(tuAddedIcuRef).toBeTruthy();
            const tuAddedIcu = langFileGerman.transUnitWithId(ID_ADDDED_ICU);
            expect(tuAddedIcu).toBeTruthy();
            expect(tuAddedIcuRef.targetContent()).toContain('<x id="ICU" equiv-text="{ fruit, select, Apple {...}');
            expect(tuAddedIcuRef.targetState()).toBe(ngx_i18nsupport_lib_1.STATE_NEW);
            expect(tuAddedIcu.sourceContent()).toContain('Apple {Apple}');
            expect(tuAddedIcu.targetContent()).toBe(tuAddedIcu.sourceContent());
            expect(tuAddedIcu.targetState()).toBe(ngx_i18nsupport_lib_1.STATE_NEW);
            done();
        });
        ...
        it('should use beautify when requested (#64, #88)', (done) => {
            ...
            xliffMergeCmd.run();
            expect(ws.writtenData()).not.toContain('ERROR');
            const unformattedContent = xml_reader_1.XmlReader.readXmlFileContent(xliffMergeCmd.generatedI18nFile('en'));
            ...
            file_util_1.FileUtil.deleteFile(xliffMergeCmd.generatedI18nFile('en'));
            ws = new writer_to_string_1.WriterToString();
            commandOut = new command_output_1.CommandOutput(ws);
            profileContent = {
                xliffmergeOptions: {
            ...
            xliffMergeCmd.run();
            expect(ws.writtenData()).not.toContain('ERROR');
            const formattedContent = xml_reader_1.XmlReader.readXmlFileContent(xliffMergeCmd.generatedI18nFile('en'));
        ...
        it('should not add empty lines when using beautify whith complex content (#97)', (done) => {
            file_util_1.FileUtil.copy(SRCDIR + 'issue97emptylines.xlf', MASTER);
            let ws = new writer_to_string_1.WriterToString();
            let commandOut = new command_output_1.CommandOutput(ws);
            let profileContent = {
                xliffmergeOptions: {
            ...
            let xliffMergeCmd = xliff_merge_1.XliffMerge.createFromOptions(commandOut, { languages: ['en', 'ru'] }, profileContent);
            xliffMergeCmd.run();
            expect(ws.writtenData()).not.toContain('ERROR');
            const formattedContent1 = xml_reader_1.XmlReader.readXmlFileContent(xliffMergeCmd.generatedI18nFile('ru'));
            ...
            xliffMergeCmd = xliff_merge_1.XliffMerge.createFromOptions(commandOut, { languages: ['en', 'ru'] }, profileContent);
            xliffMergeCmd.run();
            expect(ws.writtenData()).not.toContain('ERROR');
            const formattedContent2 = xml_reader_1.XmlReader.readXmlFileContent(xliffMergeCmd.generatedI18nFile('ru'));
        ...
        it('should not change translation state when source is unchanged but has different line ending (#128)', (done) => {
            file_util_1.FileUtil.copy(SRCDIR + 'issue128master.xlf', MASTER);
            const langfileName = WORKDIR + 'messages.en.xlf';
            file_util_1.FileUtil.copy(SRCDIR + 'issue128translated.xlf', langfileName);
            const originalContent = file_util_1.FileUtil.read(langfileName, 'UTF-8');
            const contentWithoutCRLF = originalContent.replace(/[\n\r]/g, '');
            expect(originalContent).not.toBe(contentWithoutCRLF);
            file_util_1.FileUtil.replaceContent(langfileName, contentWithoutCRLF, 'UTF-8');
            const ws = new writer_to_string_1.WriterToString();
            const commandOut = new command_output_1.CommandOutput(ws);
            const profileContent = {
                xliffmergeOptions: {
            ...
            expect(ws.writtenData()).not.toContain('ERROR');
            const langFileEnglish = readXliff(xliffMergeCmd.generatedI18nFile('en'));
            const tu = langFileEnglish.transUnitWithId('issue128');
            expect(tu.targetState()).toBe(ngx_i18nsupport_lib_1.STATE_FINAL);
            done();
        });
        describe('autotranslate via google translate', () => {
            ...
            it('should auto translate file with region code (which will be ignored)', (done) => {
                ...
                const xliffMergeCmd = xliff_merge_1.XliffMerge.createFromOptions(commandOut, { languages: ['de-de', 'en-us'] }, profileContent);
                xliffMergeCmd.run((retcode) => {
                    expect(ws.writtenData()).not.toContain('ERROR');
                    const langFileEnglish = readXliff(xliffMergeCmd.generatedI18nFile('en-us'));
            ...
            it('should detect unsupported language when using auto translate', (done) => {
                ...
                xliffMergeCmd.run((retcode) => {
                    ...
                    const langFileXy = readXliff(xliffMergeCmd.generatedI18nFile('xy'));
            ...
            it('should auto translate text containing linefeeds (originated by #78)', (done) => {
                ...
                file_util_1.FileUtil.copy(SRCDIR + 'autotranslateMaster1.xlf', MASTER);
                const ws = new writer_to_string_1.WriterToString();
                const commandOut = new command_output_1.CommandOutput(ws);
                const profileContent = {
                    xliffmergeOptions: {
            ...
            it('should auto translate english text to french containing apostrophes (originated by #94)', (done) => {
                ...
                file_util_1.FileUtil.copy(SRCDIR + 'englishToFrench.xlf', MASTER);
                const ws = new writer_to_string_1.WriterToString();
                const commandOut = new command_output_1.CommandOutput(ws);
                const profileContent = {
                    xliffmergeOptions: {
                ...
                const xliffMergeCmd = xliff_merge_1.XliffMerge.createFromOptions(commandOut, { languages: ['en', 'fr'] }, profileContent);
                xliffMergeCmd.run((retcode) => {
                    expect(ws.writtenData()).not.toContain('ERROR');
                    const langFileFrench = readXliff(xliffMergeCmd.generatedI18nFile('fr'));
...
//# sourceMappingURL=xlf12-merge.spec.js.map