...
function createEmitCallback(options) {
    ...
    if (options.annotateForClosureCompiler || options.annotationsAs === 'static fields') {
        return ({ program, targetSourceFile, writeFile, cancellationToken, emitOnlyDtsFiles, customTransformers = {}, host, options }) => require('tsickle').emitWithTsickle(program, Object.assign({}, tsickleHost, { options, moduleResolutionHost: host }), host, options, targetSourceFile, writeFile, cancellationToken, emitOnlyDtsFiles, {
            beforeTs: customTransformers.before,
            afterTs: customTransformers.after
        });
    }
    else {
        return ({ program, targetSourceFile, writeFile, cancellationToken, emitOnlyDtsFiles, customTransformers = {} }) => program.emit(targetSourceFile, writeFile, cancellationToken, emitOnlyDtsFiles, {
